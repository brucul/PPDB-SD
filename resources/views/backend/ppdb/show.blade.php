...
<div class="col-12">
    <div class="card">
        <div class="card-body">
            <form class="row" method="post" action="" id="form-status">
                <input type="hidden" name="id" value="{{ $student->id }}">
                <div class="col-sm-6">
                    <div class="form-group row">
                        <label for="status" class="col-sm-2 col-form-label">Status</label>
                        <div class="col-sm-10">
                            <select type="text" class="form-control select2 status" name="status" id="status" required>
                                <option value="0" {{ $student->status == 0 ? 'selected' : '' }} disabled>Pilih Status</option>
                                <option value="1" {{ $student->status == 1 ? 'selected' : '' }}>Lolos</option>
                                <option value="2" {{ $student->status == 2 ? 'selected' : '' }}>Tidak Lolos</option>
                            </select>
                        </div>
                    </div>
                </div>
                <div class="col-sm-6" align="right">
                    <button type="submit" class="btn btn-success mr-2"><i class="fa fa-save"></i> Update Status</button>
                    <button type="button" class="btn btn-dark btn-icon icon-left" id="btn-download"><i class="far fa-circle-down"></i> Download Berkas</button>
                </div>
            </form>
        </div>
    </div>
    <div class="card">
        <div class="card-header bg-whitesmoke">
            <div class="col-sm-12 d-flex justify-content-between">
                <div class="d-flex">
                    <h4>Jalur {{ $student->type == 1 ? 'Zonasi' : 'Prestasi' }} ({{ $student->registration_number }})</h4>
                </div>
                <h4 id="status-title">
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#form-status').submit( (e) => {
            e.preventDefault()

            $.ajax({
                url : "{{ route('be.ppdb.update') }}",
                method : "post",
                cache : false,
                contentType: false,
                processData: false,
                data : new FormData($('#form-status').get(0)),
                success : function(response) {
                    if(response.status) {
                        swal('Success', response.message, 'success')
                        $('#status-title').html(response.badge)
                    } else {
